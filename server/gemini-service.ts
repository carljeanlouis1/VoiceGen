...
export async function generateGeminiContent(options: GenerateContentOptions): Promise<GenerateContentResult> {
  try {
    ...
    const userParts: any[] = [{ text: options.prompt }];
    ...
    if (options.systemPrompt) {
      ...
      // Standard request without system prompt - always include role
      contents = [
        {
          role: "user",
          parts: userParts
    ...
    // Set the API endpoint for the Gemini 2.5 Pro model - using latest model version
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:generateContent?key=${apiKey}`;
    
    // Log the request payload for debugging (without sensitive info)
    log(`Gemini API request structure: ${JSON.stringify({
      ...payload,
      contents: payload.contents.map((c: any) => ({ role: c.role, parts: c.parts.map((p: any) => p.text ? { textLength: p.text.length } : { type: 'image' }) }))
    })}`);
    ...
    log(`Gemini API response structure: ${JSON.stringify(Object.keys(result))}`);
    
    if (!result.candidates || result.candidates.length === 0) {
      if (result.promptFeedback) {
        log(`Gemini API safety feedback: ${JSON.stringify(result.promptFeedback)}`);
        throw new Error(`Content blocked by safety settings: ${JSON.stringify(result.promptFeedback)}`);
      }
