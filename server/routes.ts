...
${allResearch.substring(0, 5000)}`;  // Limit research for conclusion to 5000 chars
  ...
  async function generateExtendedContent(
    ...
    try {
      ...
      // Ensure topic isn't too long
      if (data.topic && data.topic.length > 5000) {
        return res.status(400).json({
          error: "Topic exceeds maximum length of 5,000 characters"
        });
      }
  ...
  // Create a memory usage monitor middleware (simplified)
  app.use((req, res, next) => {
    try {
      const memoryUsage = process.memoryUsage();
      const mbUsed = Math.round(memoryUsage.rss / 1024 / 1024);
      
      // Just log high memory usage
      if (mbUsed > 300) {
        console.warn(`HIGH MEMORY USAGE: ${mbUsed} MB`);
      }
    } catch (error) {
      // Safely ignore any memory monitoring errors
